...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        Log.d(TAG, "onMapReady: Checking Build version....");
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            Log.d(TAG, "onMapReady: Checking user permission....");
            if (checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED && checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                Log.d(TAG, "onMapReady: user permission not granted !!");
                ...
                Log.d(TAG, "onMapReady: Permission granted successfully.");
            }
            locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 20, 1, locationListener);


        } else {
            Log.d(TAG, "onMapReady: build version is < M");
            locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 20, 1, locationListener);
        }
